                ...
                try:
                    ...
                    if not dusun:
                        if not skip_errors:
                            ...
                            # Skip this row if dusun not found and skip_errors is True
                            continue
                    ...
                    penduduk_data = {
                        ...
                        'dusun': dusun,  # This can now be None due to the migration fix
                        'lorong': lorong,
                        ...
                        'is_active': True,  # Set to True for imported data
                        'is_alive': True,   # Set to True for imported data
                        'citizenship': 'WNI',  # Default value
                    }
                    
                    # Debug logging
                    print(f"Creating penduduk: {penduduk_data['name']} - NIK: {penduduk_data['nik']}")
                    print(f"is_active: {penduduk_data['is_active']}, is_alive: {penduduk_data['is_alive']}")
                    ...
                    if 'RT' in df.columns and pd.notna(row['RT']) and dusun:
                    ...
                    try:
                        # Check if penduduk already exists by NIK
                        existing_penduduk = Penduduk.objects.filter(nik=penduduk_data['nik']).first()
                        if existing_penduduk:
                            # Update existing penduduk
                            for key, value in penduduk_data.items():
                                setattr(existing_penduduk, key, value)
                            existing_penduduk.save()
                            penduduk = existing_penduduk
                            print(f"Updated existing penduduk: {penduduk.name} (NIK: {penduduk.nik})")
                        else:
                            # Create new penduduk
                            penduduk = Penduduk.objects.create(**penduduk_data)
                            print(f"Successfully created penduduk: {penduduk.name} (NIK: {penduduk.nik})")
                        
                        # Verify the penduduk was created/updated correctly
                        penduduk.refresh_from_db()
                        if penduduk.is_active and penduduk.is_alive:
                            success_count += 1
                            print(f"✅ Penduduk {penduduk.name} is active and alive")
                        else:
                            print(f"❌ Penduduk {penduduk.name} is not active or alive - is_active: {penduduk.is_active}, is_alive: {penduduk.is_alive}")
                            # Force update to ensure it's active
                            penduduk.is_active = True
                            penduduk.is_alive = True
                            penduduk.save()
                            success_count += 1
                            print(f"🔧 Fixed penduduk {penduduk.name} status")
...
def quick_import(request, model_name):
    ...
    if model_name == 'penduduk':
        # Use improved import view for penduduk
        from .improved_penduduk_import import ImprovedPendudukImportView
        return ImprovedPendudukImportView.as_view()(request)
    elif model_name == 'rt':
