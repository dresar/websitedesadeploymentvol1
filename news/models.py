...
class News(models.Model):
    ...
    is_summonote = models.BooleanField(default=False, help_text='Tandai sebagai Summonote')
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            self.slug = slugify(cleaned_title)
            
            # If slug is empty (non-latin characters), use UUID
            if not self.slug:
                import uuid
...
class Announcement(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Generate slug if it doesn't exist or is empty
        if not self.slug or self.slug == '':
            base_slug = slugify(self.title)
            if not base_slug:  # If title is empty or only special chars
                base_slug = 'announcement'
            
            slug = base_slug
            counter = 1
            # Make sure slug is unique
            # If object has pk, exclude it from the query (for updates)
            if self.pk:
                while Announcement.objects.filter(slug=slug).exclude(pk=self.pk).exists():
                    slug = f"{base_slug}-{counter}"
                    counter += 1
            else:
                # For new objects, check without excluding
