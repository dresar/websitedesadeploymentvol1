...
class NewsForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            'status', 'priority', 'is_featured', 'is_breaking', 'is_summonote', 'published_date', 'scheduled_date',
        ...
        widgets = {
            ...
            'excerpt': SummernoteWidget(attrs={
                'class': 'form-control',
                'maxlength': '300',
            ...
            'is_summonote': forms.CheckboxInput(attrs={
                'class': 'form-check-input'
            }),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            if self.instance.published_date:
                # Convert to local timezone and format for HTML5 datetime-local
                local_published = self.instance.published_date.astimezone()
                self.fields['published_date'].initial = local_published.strftime('%Y-%m-%dT%H:%M')
            if self.instance.scheduled_date:
                # Convert to local timezone and format for HTML5 datetime-local
                local_scheduled = self.instance.scheduled_date.astimezone()
                self.fields['scheduled_date'].initial = local_scheduled.strftime('%Y-%m-%dT%H:%M')
        else:
            # Set default values for new instances
            from django.utils import timezone
            now = timezone.now()
            self.fields['published_date'].initial = now.strftime('%Y-%m-%dT%H:%M')
