...
def admin_panel_dashboard(request):
    ...
    context = {
        ...
        'active_submenu': 'posyandu_dashboard',
...
def admin_panel_export_immunizations(request):
    """Export data imunisasi ke PDF"""
    from .pdf_exports import export_immunizations_pdf
    
    # Get data
    immunizations = Immunization.objects.select_related('patient', 'posyandu').order_by('-immunization_date')
    
    return export_immunizations_pdf(immunizations)
...
def admin_panel_health_record_update(request, record_id):
    ...
    context = {
        ...
        'record': record,
        'locations': locations,
        'page_title': f'Edit Rekam Kesehatan - {record.patient.name}',
...
def admin_panel_immunization_update(request, immunization_id):
    ...
    context = {
        ...
        'locations': locations,
        'page_title': f'Edit Data Imunisasi - {immunization.patient.name}',
...
def admin_panel_nutrition_update(request, nutrition_id):
    ...
    context = {
        ...
        'nutrition': nutrition,
        'locations': locations,
        'page_title': f'Edit Data Gizi - {nutrition.patient.name}',
...
def admin_panel_kader_update(request, kader_id):
    ...
    context = {
        ...
        'kader': kader,
        'posyandu_locations': posyandu_locations,
        'page_title': f'Edit Kader Posyandu - {kader.penduduk.name}',
...
def admin_panel_ibu_hamil_update(request, ibu_hamil_id):
    ...
    context = {
        ...
        'active_submenu': 'ibu_hamil',
        'ibu_hamil': ibu_hamil,
        'page_title': f'Edit Data Ibu Hamil - {ibu_hamil.penduduk.name if ibu_hamil.penduduk else "Unknown"}',
...
def admin_panel_stunting_update(request, stunting_id):
    ...
    context = {
        ...
        'stunting': stunting,
        'locations': locations,
        'page_title': f'Edit Data Stunting - {stunting.balita.name}',
...
def admin_panel_health_record_detail(request, record_id):
    ...
    context = {
        ...
        'active_submenu': 'health_records',
        'record': record,
        'page_title': f'Detail Rekam Kesehatan - {record.patient.name}',
...
def admin_panel_immunization_detail(request, immunization_id):
    ...
    context = {
        ...
        'active_submenu': 'immunizations',
        'immunization': immunization,
        'page_title': f'Detail Data Imunisasi - {immunization.patient.name}',
...
def admin_panel_nutrition_detail(request, nutrition_id):
    ...
    context = {
        ...
        'active_submenu': 'nutrition',
        'nutrition': nutrition,
        'page_title': f'Detail Data Gizi - {nutrition.patient.name}',
...
def admin_panel_ibu_hamil_detail(request, ibu_hamil_id):
    ...
    context = {
        ...
        'active_submenu': 'ibu_hamil',
        'ibu_hamil': ibu_hamil,
        'page_title': f'Detail Ibu Hamil - {ibu_hamil.penduduk.name if ibu_hamil.penduduk else "Unknown"}',
...
def admin_panel_nutrition_update(request, nutrition_id):
    ...
    context = {
        ...
        'active_submenu': 'nutrition',
        'nutrition': nutrition,
        'page_title': f'Edit Data Gizi - {nutrition.patient.name if nutrition.patient else "Unknown"}',
...
def admin_panel_kader_detail(request, kader_id):
    ...
    try:
        kader = get_object_or_404(PosyanduKader.objects.select_related('penduduk', 'posyandu'), id=kader_id)
        schedules = []
        ...
        context = {
            ...
            'kader': kader,
            'schedules': schedules,
            'page_title': f'Detail Kader - {kader.penduduk.name if kader.penduduk else "Unknown"}',
            'page_subtitle': 'Detail kader posyandu'
        }
        return render(request, 'admin_panel/posyandu/kader_detail.html', context)
        
    except PosyanduKader.DoesNotExist:
        messages.error(request, 'Kader tidak ditemukan.')
        return redirect('admin_panel:posyandu:kaders')
    except Exception as e:
...
def admin_panel_stunting_detail(request, stunting_id):
    ...
    context = {
        ...
        'active_submenu': 'stunting',
        'stunting': stunting,
        'page_title': f'Detail Data Stunting - {stunting.balita.name if stunting.balita else "Unknown"}',
...
def admin_panel_stunting_update(request, stunting_id):
    ...
    context = {
        'stunting': stunting,
        'page_title': f'Edit Data Stunting - {stunting.balita.name if stunting.balita else "Unknown"}',
...
@login_required
def admin_panel_export_health_records(request):
    """Export data rekam kesehatan ke PDF"""
    from .pdf_exports import export_health_records_pdf
    
    # Get data
    records = HealthRecord.objects.select_related('patient', 'posyandu').order_by('-visit_date')
    
    return export_health_records_pdf(records)
...
def admin_panel_export_nutrition_data(request):
    """Export data gizi ke PDF"""
    from .pdf_exports import export_nutrition_data_pdf
    
    # Get data
    nutrition_data = NutritionData.objects.select_related('patient', 'posyandu').order_by('-measurement_date')
    
    return export_nutrition_data_pdf(nutrition_data)


@login_required
def admin_panel_export_stunting_data(request):
    """Export data stunting ke PDF"""
    from .pdf_exports import export_stunting_data_pdf
    
    # Get data
    stunting_data = StuntingData.objects.select_related('balita').order_by('-tanggal_ukur')
    
    return export_stunting_data_pdf(stunting_data)
