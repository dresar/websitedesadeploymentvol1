...
def public_posyandu_locations_api(request):
    """API to get active posyandu locations with pagination"""
    try:
        from django.core.paginator import Paginator
            ...
            'results': [
                {
                    ...
                    'established_date': loc.established_date.strftime('%Y-%m-%d') if loc.established_date else None
                }
                for loc in page_obj
            ],
            'pagination': {
...
def public_posyandu_schedules_api(request):
    """API publik untuk mendapatkan jadwal posyandu dengan pagination"""
    try:
        ...
        schedules = PosyanduSchedule.objects.filter(
            ...
            is_completed=False
        ).select_related('location').order_by('schedule_date', 'start_time')
            ...
            'results': [
                {
                    ...
                    'location_name': schedule.location.name if schedule.location else 'Lokasi Tidak Tersedia',
                    'location_address': schedule.location.address if schedule.location else '',
                    ...
                    'end_time': schedule.end_time.strftime('%H:%M') if schedule.end_time else '13:00',
                    'description': schedule.description or '',
                    'target_participants': schedule.target_participants or 0,
...
def api_statistics(request):
    ...
    try:
        data = {
            ...
            'total_kader': PosyanduKader.objects.filter(status='aktif').count(),
            'total_ibu_hamil': IbuHamil.objects.filter(status_aktif=True).count(),
            'total_balita': HealthRecord.objects.filter(patient_type='balita').count(),
            'total_immunizations': Immunization.objects.count(),
            'total_health_records': HealthRecord.objects.count(),
            'total_nutrition_data': NutritionData.objects.count(),
            'total_stunting_data': StuntingData.objects.count(),
            ...
            'recent_immunizations': Immunization.objects.filter(
                created_at__gte=timezone.now() - timedelta(days=1)
            ).count(),
            'recent_nutrition_data': NutritionData.objects.filter(
                created_at__gte=timezone.now() - timedelta(days=1)
            ).count()
        }
        return JsonResponse(data)
...
def api_ibu_hamil_list(request):
    ...
    try:
        ...
        ibu_hamil_list = IbuHamil.objects.all().select_related('penduduk', 'posyandu').order_by('-tanggal_hpht', '-id')
        ...
        all_ibu_hamil = ibu_hamil_list
        total = all_ibu_hamil.count()
        trimester1 = all_ibu_hamil.filter(usia_kehamilan__lte=12).count()
        trimester2 = all_ibu_hamil.filter(usia_kehamilan__gt=12, usia_kehamilan__lte=27).count()
        trimester3 = all_ibu_hamil.filter(usia_kehamilan__gt=27).count()
        
        # Pagination
        page_size = int(request.GET.get('page_size', 10))
        page_number = int(request.GET.get('page', 1))
        
        paginator = Paginator(ibu_hamil_list, page_size)
        ...
        data = {
            'results': [
                {
                    'id': ibu.id,
                    'penduduk_name': ibu.penduduk.name if ibu.penduduk else None,
    ...
    try:
        if request.method == 'GET':
            ...
            return JsonResponse({
                ...
                'penduduk_name': ibu_hamil.penduduk.name if ibu_hamil.penduduk else None,
                'penduduk_nik': ibu_hamil.penduduk.nik if ibu_hamil.penduduk else None,
                'penduduk_birth_date': None,
                'penduduk_gender': None,
...
# Immunization API Functions (Moved to line 2275 with pagination support)
...
def api_kader_list(request):
    ...
    try:
        ...
        # Calculate jabatan distribution for chart
        jabatan_distribution = {}
        for kader in kaders:
            jabatan = kader.jabatan or 'Tidak Ada'
            jabatan_distribution[jabatan] = jabatan_distribution.get(jabatan, 0) + 1
        ...
        data = {
            'results': [
                {
                    'id': kader.id,
                    'name': kader.penduduk.name if kader.penduduk else None,
                    ...
                    'avatar': '/static/images/defaults/avatar-placeholder.svg'  # Penduduk model doesn't have photo field
                }
            ...
            'statistics': {
                ...
                'jabatan_distribution': jabatan_distribution
            },
            'pagination': {
                'current_page': page_obj.number,
                'total_pages': paginator.num_pages,
...
def api_penduduk_search(request):
    ...
    try:
        # Import Penduduk model - use references model
        ...
        # Build base queryset - check if model has is_active and is_alive fields
        ...
        # Check if model has related fields
        if hasattr(Penduduk, 'dusun') and hasattr(Penduduk, 'lorong'):
            penduduk_list = penduduk_list.select_related('dusun', 'lorong')
        
        # Apply search query if provided
        if query and len(query) >= 2:
            # Build Q objects dynamically based on available fields
            # Use 'name' field for references Penduduk model
            q_objects = Q(name__icontains=query) | Q(nik__icontains=query)
            
            # Add phone fields if they exist
            if hasattr(Penduduk, 'telepon'):
                q_objects |= Q(telepon__icontains=query)
            if hasattr(Penduduk, 'phone_number'):
                q_objects |= Q(phone_number__icontains=query)
            if hasattr(Penduduk, 'mobile_number'):
                q_objects |= Q(mobile_number__icontains=query)
                
            penduduk_list = penduduk_list.filter(q_objects)
        elif query and len(query) < 2:
            return JsonResponse({'success': True, 'data': []})
        
        # Apply additional filters only if fields exist
        if gender and hasattr(Penduduk, 'gender'):
            penduduk_list = penduduk_list.filter(gender=gender)
        
        # Add age group filter if provided (only if birth_date field exists)
        if hasattr(Penduduk, 'birth_date') or hasattr(Penduduk, 'tanggal_lahir'):
            birth_field = 'birth_date' if hasattr(Penduduk, 'birth_date') else 'tanggal_lahir'
            
            if age_group == 'balita':
                # Balita: 0-5 tahun (0-60 bulan) - filter by birth_date
                from datetime import date, timedelta
                five_years_ago = date.today() - timedelta(days=5*365)
                penduduk_list = penduduk_list.filter(**{f'{birth_field}__gte': five_years_ago})
            elif age_group == 'ibu_hamil':
                # Ibu hamil: perempuan 15-40 tahun - only females
                from datetime import date, timedelta
                fifteen_years_ago = date.today() - timedelta(days=int(15*365.25))
                forty_years_ago = date.today() - timedelta(days=int(40*365.25))
                if hasattr(Penduduk, 'gender'):
                    penduduk_list = penduduk_list.filter(
                        gender='P',  # Only females
                        **{f'{birth_field}__lte': fifteen_years_ago, f'{birth_field}__gte': forty_years_ago}  # 15-40 years old
                    )
                else:
                    # If no gender field, just filter by age
                    penduduk_list = penduduk_list.filter(
                        **{f'{birth_field}__lte': fifteen_years_ago, f'{birth_field}__gte': forty_years_ago}  # 15-40 years old
                    )
        else:
            # If no birth_date field available, show warning but still return results
            if age_group in ['balita', 'ibu_hamil']:
                # Continue with normal search but add warning to response
                pass
            ...
            if age_min:
                ...
                penduduk_list = penduduk_list.filter(**{f'{birth_field}__lte': max_birth_date})
            if age_max:
                from datetime import date, timedelta
                min_birth_date = date.today() - timedelta(days=int(age_max) * 365)
                penduduk_list = penduduk_list.filter(**{f'{birth_field}__gte': min_birth_date})
        ...
        for penduduk in penduduk_list:
            try:
                ...
                age = 0
                birth_date = None
                if hasattr(penduduk, 'birth_date'):
                    birth_date = penduduk.birth_date
                elif hasattr(penduduk, 'tanggal_lahir'):
                    birth_date = penduduk.tanggal_lahir
                ...
                # Get name field
                nama = getattr(penduduk, 'name', '') or 'N/A'
                
                # Get phone - check multiple possible field names
                phone = ''
                if hasattr(penduduk, 'telepon'):
                    phone = penduduk.telepon or ''
                elif hasattr(penduduk, 'phone_number'):
                    phone = penduduk.phone_number or ''
                elif hasattr(penduduk, 'no_hp'):
                    phone = penduduk.no_hp or ''
                elif hasattr(penduduk, 'mobile_number'):
                    phone = penduduk.mobile_number or ''
                
                # Get address
                full_address = getattr(penduduk, 'address', '') or ''
                    
                # Add dusun and lorong if they exist
                if hasattr(penduduk, 'dusun') and penduduk.dusun:
                    dusun_name = getattr(penduduk.dusun, 'name', '') or getattr(penduduk.dusun, 'nama', '')
                    if full_address:
                        full_address = f"{full_address}, Dusun {dusun_name}"
                    else:
                        full_address = f"Dusun {dusun_name}"
                        
                if hasattr(penduduk, 'lorong') and penduduk.lorong:
                    lorong_name = getattr(penduduk.lorong, 'name', '') or getattr(penduduk.lorong, 'nama', '')
                    full_address = f"{full_address}, Lorong {lorong_name}"
                ...
                # Get RT and RW - check for both number fields and objects
                rt_value = ''
                if hasattr(penduduk, 'rt_number'):
                    rt_value = str(penduduk.rt_number) if penduduk.rt_number else ''
                elif hasattr(penduduk, 'rt') and penduduk.rt:
                    # If rt is an object, convert to string
                    rt_value = str(penduduk.rt)
                    
                rw_value = ''
                if hasattr(penduduk, 'rw_number'):
                    rw_value = str(penduduk.rw_number) if penduduk.rw_number else ''
                elif hasattr(penduduk, 'rw') and penduduk.rw:
                    # If rw is an object, convert to string
                    rw_value = str(penduduk.rw)
                ...
                penduduk_data = {
                    ...
                    'dusun': getattr(penduduk.dusun, 'name', '') or getattr(penduduk.dusun, 'nama', '') if hasattr(penduduk, 'dusun') and penduduk.dusun else '',
                    'lorong': getattr(penduduk.lorong, 'name', '') or getattr(penduduk.lorong, 'nama', '') if hasattr(penduduk, 'lorong') and penduduk.lorong else '',
                    'rt': rt_value,
                    'rw': rw_value,
                    'education': penduduk.get_education_display() if hasattr(penduduk, 'get_education_display') and hasattr(penduduk, 'education') and penduduk.education else '',
                    'occupation': getattr(penduduk, 'occupation', ''),
                    'religion': penduduk.get_religion_display() if hasattr(penduduk, 'get_religion_display') and hasattr(penduduk, 'religion') and penduduk.religion else '',
                    'marital_status': penduduk.get_marital_status_display() if hasattr(penduduk, 'get_marital_status_display') and hasattr(penduduk, 'marital_status') and penduduk.marital_status else '',
                    'photo': penduduk.photo.url if hasattr(penduduk, 'photo') and penduduk.photo else '/static/images/defaults/avatar-placeholder.svg',
                    'is_active': getattr(penduduk, 'is_active', True),
                    'is_alive': getattr(penduduk, 'is_alive', True),
                    'created_at': penduduk.created_at.strftime('%Y-%m-%d %H:%M') if hasattr(penduduk, 'created_at') and penduduk.created_at else None,
                    'updated_at': penduduk.updated_at.strftime('%Y-%m-%d %H:%M') if hasattr(penduduk, 'updated_at') and penduduk.updated_at else None
                }
                ...
                # Add minimal data for this penduduk - robust fallback
                try:
                    nama = getattr(penduduk, 'name', '') or 'N/A'
                    nik = getattr(penduduk, 'nik', '')
                    phone = getattr(penduduk, 'phone_number', '') or getattr(penduduk, 'mobile_number', '') or ''
                    ...
                    print(f"Failed to add even minimal data for penduduk {penduduk.id}: {str(e)}")
                    pass  # Skip this record if even basic data can't be extracted
        ...
        # Add warning if age_group filter was requested but not available
        if age_group in ['balita', 'ibu_hamil'] and not (hasattr(Penduduk, 'birth_date') or hasattr(Penduduk, 'tanggal_lahir')):
            response_data['warning'] = f'Filter {age_group} tidak tersedia karena data tanggal lahir tidak ada. Menampilkan semua hasil pencarian.'
    ...
    try:
        ...
        data = {
            'results': [
                {
                    'id': stunting.id,
                    'child_name': stunting.balita.name if stunting.balita else None,
    ...
    try:
        if request.method == 'GET':
            ...
            return JsonResponse({
                ...
                'child_name': stunting_data.balita.name if stunting_data.balita else None,
...
def api_immunization_list(request):
    ...
    try:
        ...
        immunizations = Immunization.objects.all().select_related('patient', 'posyandu').order_by('-immunization_date', '-id')
        ...
        paginator = Paginator(immunizations, page_size)
        ...
        data = {
            'results': [
                {
                    'id': immunization.id,
                    'child_name': immunization.patient.name if immunization.patient else 'Unknown',
        ...
        import traceback
        print(f"Error in api_immunization_list: {str(e)}")
        print(traceback.format_exc())
        return JsonResponse({'error': str(e)}, status=500)
...
def api_nutrition_data_list(request):
    ...
    try:
        ...
        nutrition_data = NutritionData.objects.all().select_related('patient', 'posyandu').order_by('-measurement_date', '-id')
        ...
        paginator = Paginator(nutrition_data, page_size)
        ...
        import traceback
        print(f"Error in api_nutrition_data_list: {str(e)}")
        print(traceback.format_exc())
        return JsonResponse({'error': str(e)}, status=500)
