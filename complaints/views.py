...
def complaint_form_view(request):
    ...
    # Get hero image from core context processor
    try:
        from core.models import HeroImage
        hero_image = HeroImage.objects.filter(page='complaints', is_active=True, image__isnull=False).exclude(image='').first()
        if hero_image and hero_image.image:
            hero_image_url = hero_image.image.url
            hero_title = hero_image.name
            hero_subtitle = f"Website Desa Pulosarok - {hero_image.get_page_display()}"
        else:
            hero_image_url = None
            hero_title = None
            hero_subtitle = None
    ...
    return render(request, 'public/complaints/index.html', context)
...
def complaint_success_view(request, complaint_id):
    ...
    return render(request, 'public/complaints/success.html', context)
...
def complaint_tracking_view(request):
    ...
    return render(request, 'public/complaints/tracking.html', context)
...
def complaint_detail_view(request, complaint_id):
    ...
    return render(request, 'public/complaints/detail.html', context)


# Aliases for admin panel compatibility
def complaint_detail(request, pk):
    """Alias for complaint_detail_view for admin panel compatibility"""
    # Convert pk (id) to complaint_id for the original function
    try:
        ...
        # Initialize form with current admin notes
        admin_notes_form = AdminNotesForm(initial={'admin_notes': complaint.admin_notes})
        
        context = {
            'complaint': complaint,
            'updates': updates,
            'admin_notes_form': admin_notes_form,
            'page_title': f'Detail Pengaduan {complaint.complaint_id}',
            'active_menu': 'complaints',
            'active_submenu': 'complaints_list',
        }
...
def complaint_edit(request, pk):
    ...
    # Create form with complaint data
    from .forms import ComplaintForm, ComplaintAdminNotesForm
    complaint_form = ComplaintForm(instance=complaint)
    admin_notes_form = ComplaintAdminNotesForm(instance=complaint)
    ...
    context = {
        ...
        'page_title': f'Edit Pengaduan {complaint.complaint_id}',
        'active_menu': 'complaints',
        'active_submenu': 'complaints_list',
    }
    return render(request, 'admin_panel/complaints/edit.html', context)

def complaint_update(request, pk):
    """Alias for admin_complaint_update for admin panel compatibility"""
    return admin_complaint_update(request, pk)

def complaint_delete(request, pk):
    """Alias for admin_complaint_delete for admin panel compatibility"""
    return admin_complaint_delete(request, pk)

def complaint_add_update(request, pk):
    """Alias for admin_complaint_add_update for admin panel compatibility"""
    return admin_complaint_add_update(request, pk)

@login_required
@user_passes_test(is_admin)
@require_http_methods(["POST"])
def complaint_update_admin_notes(request, pk):
    """Update admin notes for complaint"""
    complaint = get_object_or_404(Complaint, pk=pk)
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'message': f'Gagal memperbarui catatan admin: {str(e)}'
        }, status=400)

def complaint_categories_list(request):
    """Alias for admin_complaint_categories_list for admin panel compatibility"""
...
def complaints_dashboard(request):
    ...
    context = {
        ...
        'active_menu': 'complaints',
        'active_submenu': 'complaints_dashboard',
    }
...
def admin_complaint_detail(request, pk):
    ...
    context = {
        ...
        'active_menu': 'complaints',
        'active_submenu': 'complaints_detail',
        'complaint': complaint,
        'updates': updates,
    }
    return render(request, 'admin_panel/complaints/detail.html', context)
