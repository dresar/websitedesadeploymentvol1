...
class MaintenanceModeMiddleware:
    ...
    def should_skip_maintenance(self, request):
        ...
        # Special case: Skip for superuser in DEBUG mode (for testing)
        if settings.DEBUG and hasattr(request, 'user') and request.user.is_authenticated and request.user.is_superuser:
            # Check if there's a special parameter to force maintenance mode
            if request.GET.get('force_maintenance') == 'true':
                return False
    ...
    def render_maintenance_page(self, request, website_settings):
        ...
        context = {
            ...
            'site_tagline': website_settings.site_description or 'Website Desa Pulosarok',
