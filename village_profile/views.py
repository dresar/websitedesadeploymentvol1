...
def dashboard_view(request):
    ...
    try:
        ...
        # Create fallback village_profile if none exists
        if not village_profile:
            class FallbackVillageProfile:
                def __init__(self):
                    self.name = 'Desa Pulosarok'
                    self.district = 'Singkil'
                    self.regency = 'Kabupaten Aceh Singkil'
                    self.province = 'Aceh'
                    self.postal_code = '92700'
                    self.village_head = 'Kepala Desa'
                    self.area = '10.5'
                    self.description = 'Desa Pulosarok adalah desa yang terletak di Kecamatan Singkil, Kabupaten Aceh Singkil, Aceh.'
                    self.vision = 'Menjadi desa yang maju, mandiri, dan sejahtera.'
                    self.mission = 'Meningkatkan kesejahteraan masyarakat\nMengembangkan potensi desa\nMemperkuat tata kelola pemerintahan'
                    self.phone = ''
                    self.email = ''
                    self.logo = None
                    self.profile_image = None
                    self.mission_list = ['Meningkatkan kesejahteraan masyarakat', 'Mengembangkan potensi desa', 'Memperkuat tata kelola pemerintahan']
                    
                def __bool__(self):
                    return True
                    
            village_profile = FallbackVillageProfile()
...
def officials_view(request):
    ...
    try:
        ...
        village_profile = VillageProfile.objects.filter(is_active=True).first()
        officials_queryset = village_profile.officials.filter(is_active=True).order_by('display_order', 'position') if village_profile else []
        
        # Pagination
        paginator = Paginator(officials_queryset, 12)  # 12 items per page
...
def official_detail_view(request, official_id):
    """Official detail page view"""
    try:
        official = get_object_or_404(VillageOfficial, id=official_id, is_active=True)
        village_profile = VillageProfile.objects.filter(is_active=True).first()
        
        context = {
            'village_profile': village_profile,
            'official': official,
            'page_title': f'Detail {official.name}'
        }
        return render(request, 'public/village_profile/official_detail.html', context)
    except Exception as e:
        return dashboard_view(request)
