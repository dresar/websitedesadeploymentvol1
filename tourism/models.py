...
class TourismGallery(models.Model):
    ...
    def __str__(self):
        title = self.title or "Gambar"
        location = self.tourism_location.title if self.tourism_location else "No Location"
...
class TourismReview(models.Model):
    ...
    # Media
    image = models.ImageField(upload_to='tourism/reviews/', null=True, blank=True, verbose_name="Gambar Review")
...
class TourismPackage(models.Model):
    ...
    currency = models.CharField(max_length=3, default='IDR', blank=True, verbose_name="Mata Uang")
    
    # Contact info
    whatsapp = models.CharField(max_length=20, blank=True, null=True, verbose_name="Nomor WhatsApp")
    website = models.URLField(blank=True, null=True, verbose_name="Website/Portofolio")
    ...
    # Package features
    includes = models.TextField(default='[]', blank=True, verbose_name="Yang Termasuk", help_text='Included items in JSON format')
    excludes = models.TextField(default='[]', blank=True, verbose_name="Yang Tidak Termasuk", help_text='Excluded items in JSON format')
    itinerary = models.TextField(default='[]', blank=True, verbose_name="Itinerary", help_text='Itinerary in JSON format')
...
class TourismFAQ(models.Model):
    tourism_location = models.ForeignKey(TourismLocation, on_delete=models.CASCADE, related_name='faqs', verbose_name="Lokasi Wisata", null=True, blank=True)
    ...
    def __str__(self):
        location = self.tourism_location.title if self.tourism_location else "FAQ Umum"
        return f"{self.question[:50]}... - {location}"
...
class TourismSettings(models.Model):
    ...
    # Pagination Settings
    locations_per_page = models.PositiveIntegerField(default=10, verbose_name="Lokasi per Halaman")
