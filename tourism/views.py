...
def tourism_dashboard(request):
    ...
    # Get hero media from dashboard config first
    ...
    # If no hero image from dashboard config, try to get from HeroImage model
    if not hero_image:
        from core.models import HeroImage
        try:
            hero_image_obj = HeroImage.objects.filter(page='tourism', is_active=True).first()
            if hero_image_obj:
                hero_image = hero_image_obj.image
        except:
            pass
    ...
    # Get statistics
    total_locations = TourismLocation.objects.filter(status='published', is_active=True).count()
    featured_locations = TourismLocation.objects.filter(status='published', featured=True, is_active=True).count()
    ...
    avg_rating = TourismLocation.objects.filter(
        status='published', is_active=True
    ).aggregate(avg_rating=Avg('ratings__rating'))['avg_rating'] or 0
    ...
    context = {
        ...
        'featured_locations_list': TourismLocation.objects.filter(
...
def tourism_list(request):
    ...
    # Validasi field yang diizinkan untuk sorting
    allowed_sort_fields = ['-created_at', 'created_at', '-updated_at', 'updated_at', 
                          'title', '-title', 'status', '-status', 'featured', '-featured']
...
def tourism_detail(request, slug):
    ...
    # Gallery - only include items that have images
    gallery = location.gallery.filter(is_active=True, image__isnull=False).exclude(image='').order_by('order')
    
    # Reviews with pagination
    reviews_query = location.reviews.filter(is_approved=True).order_by('-created_at')
    paginator = Paginator(reviews_query, 10)  # 10 reviews per page
    page_number = request.GET.get('page_reviews', 1)
    try:
    ...
    rating_counts = location.reviews.filter(is_approved=True).values('rating').annotate(count=Count('rating'))
    total_reviews = sum(item['count'] for item in rating_counts)
    
    rating_breakdown = []
    for i in range(5, 0, -1):
        count = next((item['count'] for item in rating_counts if item['rating'] == i), 0)
...
def category_detail(request, category_id):
    ...
    related_categories = TourismCategory.objects.filter(
        is_active=True
...
def package_list(request):
    ...
    # Calculate package type statistics
    day_trip_packages = packages.filter(package_type='day_trip').count()
    weekend_packages = packages.filter(package_type='weekend').count()
    ...
    context = {
        ...
        'day_trip_packages': day_trip_packages,
        'weekend_packages': weekend_packages,
...
def package_detail(request, package_id):
    ...
    # Gallery - only include items that have images
    gallery = package.gallery.filter(is_active=True, image__isnull=False).exclude(image='').order_by('order')
...
def package_detail_by_slug(request, slug):
    ...
    # Calculate real rating breakdown efficiently
    if settings.enable_real_statistics:
        from django.db.models import Count
        rating_counts = TourismReview.objects.filter(
            tourism_location=package.tourism_location,
            is_approved=True
    ...
    context = {
        'package': package,
        'gallery': gallery,  # Fixed: use 'gallery' instead of 'galleries'
        'related_packages': related_packages,
...
def add_review(request, slug):
    ...
    if request.method == 'POST':
        form = AnonymousReviewForm(request.POST, request.FILES)
        if form.is_valid():
            ...
            review = TourismReview.objects.create(
                ...
                title=form.cleaned_data.get('title') or f"Review dari {form.cleaned_data['visitor_name']}",
                comment=form.cleaned_data['comment'],
                image=form.cleaned_data.get('image'),
                is_approved=False  # Require approval for all reviews
            )
            messages.success(request, '✅ Review Anda berhasil dikirim dan menunggu persetujuan admin.')
            return redirect('tourism:location_detail', slug=location.slug)
        else:
            messages.error(request, '❌ Terjadi kesalahan saat mengirim review. Silakan cek form Anda.')
    else:
...
def admin_dashboard(request):
    ...
    # Calculate average rating
    avg_rating = TourismReview.objects.filter(is_approved=True).aggregate(Avg('rating'))['rating__avg'] or 0
    ...
    for rating in range(1, 6):
        count = TourismReview.objects.filter(rating=rating, is_approved=True).count()
        rating_distribution.append({'rating': rating, 'count': count})
    
    # Calculate percentages
    total_approved_reviews = TourismReview.objects.filter(is_approved=True).count()
    if total_approved_reviews > 0:
        for rating in range(1, 6):
            count = TourismReview.objects.filter(rating=rating, is_approved=True).count()
            percentage = round((count / total_approved_reviews) * 100, 1)
    ...
    context = {
        'active_menu': 'tourism',
        'active_submenu': 'tourism_dashboard',
        'total_locations': total_locations,
        ...
        'rating_percentages': json.dumps(rating_percentages),
    }
...
def admin_location_detail(request, location_id):
    ...
    context = {
        'location': location,
        'gallery': location.gallery.filter(image__isnull=False).exclude(image=''),
...
def admin_package_detail(request, package_id):
    ...
    context = {
        'package': package,
        'gallery': package.gallery.filter(image__isnull=False).exclude(image=''),
    }
...
def admin_panel_dashboard(request):
    ...
    # Rating statistics - Use both TourismRating and TourismReview
    # Calculate average rating from TourismRating
    avg_rating_from_ratings = TourismRating.objects.aggregate(Avg('rating'))['rating__avg'] or 0
    
    # Calculate average rating from TourismReview (approved reviews only)
    avg_rating_from_reviews = TourismReview.objects.filter(is_approved=True).aggregate(Avg('rating'))['rating__avg'] or 0
    
    # Use the higher of the two averages, or TourismRating if both exist
    if TourismRating.objects.exists():
        avg_rating = avg_rating_from_ratings
        rating_distribution = TourismRating.objects.values('rating').annotate(
            count=Count('rating')
        ).order_by('rating')
    else:
        avg_rating = avg_rating_from_reviews
        rating_distribution = TourismReview.objects.filter(is_approved=True).values('rating').annotate(
            count=Count('rating')
        ).order_by('rating')
    ...
    for item in rating_distribution:
        rating_percentages[item['rating']] = round((item['count'] / total_ratings * 100), 1) if total_ratings > 0 else 0
...
def admin_panel_location_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('admin_panel:tourism_locations_list')
        else:
            messages.error(request, 'Terjadi kesalahan saat menambahkan lokasi. Silakan periksa form.')
    else:
        form = TourismLocationForm()
...
def admin_panel_location_detail(request, location_id):
    """Detail lokasi wisata"""
    location = get_object_or_404(TourismLocation, id=location_id)
    galleries = TourismGallery.objects.filter(tourism_location=location, image__isnull=False).exclude(image='')
    ...
    context = {
        'active_menu': 'tourism',
        'active_submenu': 'locations_detail',
        'location': location,
        'galleries': galleries,
        'reviews': reviews,
        'total_reviews': total_reviews,
...
def admin_panel_location_update(request, location_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Terjadi kesalahan saat memperbarui lokasi. Silakan periksa form.')
    else:
        form = TourismLocationForm(instance=location)
...
def admin_panel_package_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Handle multi-image gallery upload dengan title dan description
            gallery_images = request.FILES.getlist('gallery_images')
            
            # Cari semua gallery_title_* dan gallery_description_* keys
            gallery_data = {}
            for key in request.POST.keys():
                if key.startswith('gallery_title_'):
                    counter = key.replace('gallery_title_', '')
                    if counter not in gallery_data:
                        gallery_data[counter] = {}
                    gallery_data[counter]['title'] = request.POST.get(key, '')
                elif key.startswith('gallery_description_'):
                    counter = key.replace('gallery_description_', '')
                    if counter not in gallery_data:
                        gallery_data[counter] = {}
                    gallery_data[counter]['description'] = request.POST.get(key, '')
            
            # Save gallery images dengan data yang sesuai
            for index, image_file in enumerate(gallery_images):
                # Coba match dengan counter yang ada
                counter_key = str(index + 1)
                gallery_info = gallery_data.get(counter_key, {})
                
                TourismPackageGallery.objects.create(
                    package=package,
                    image=image_file,
                    title=gallery_info.get('title', f"Gambar {index + 1}"),
                    description=gallery_info.get('description', ''),
                    order=index
                )
            
            messages.success(request, 'Paket wisata berhasil ditambahkan!')
            return redirect('admin_panel:tourism_packages_list')
        else:
            messages.error(request, 'Terjadi kesalahan saat menambahkan paket. Silakan periksa form.')
    else:
        form = TourismPackageForm()
...
def admin_panel_package_detail(request, package_id):
    """Detail paket wisata"""
    package = get_object_or_404(TourismPackage, id=package_id)
    galleries = TourismPackageGallery.objects.filter(package=package, image__isnull=False).exclude(image='')
...
def admin_panel_package_update(request, package_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            gallery_to_delete = request.POST.get('gallery_to_delete', '')
            if gallery_to_delete:
                gallery_ids = [int(id) for id in gallery_to_delete.split(',') if id.strip()]
                TourismPackageGallery.objects.filter(id__in=gallery_ids).delete()
            
            # Handle new multi-image gallery upload dengan title dan description
            gallery_images = request.FILES.getlist('gallery_images')
            ...
            if gallery_images:
                ...
                # Save gallery images dengan data yang sesuai
                current_max_order = TourismPackageGallery.objects.filter(package=package).count()
                for index, image_file in enumerate(gallery_images):
                    counter_key = str(index + 1)
                    gallery_info = gallery_data.get(counter_key, {})
                    ...
                    TourismPackageGallery.objects.create(
                        package=package,
                        image=image_file,
                        title=gallery_info.get('title', f"Gambar {current_max_order + index + 1}"),
                        description=gallery_info.get('description', ''),
            ...
            messages.error(request, 'Terjadi kesalahan saat memperbarui paket. Silakan periksa form.')
    else:
        form = TourismPackageForm(instance=package)
    ...
    context = {
        'form': form,
        'package': package,
        'title': f'Edit Paket: {package.title}',
        'locations': locations,
    }
    return render(request, 'admin_panel/tourism/package_form.html', context)
...
def admin_panel_event_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('admin_panel:tourism_events_list')
        else:
            messages.error(request, 'Terjadi kesalahan saat menambahkan event. Silakan periksa form.')
    else:
        form = TourismEventForm()
    
    context = {
        'active_menu': 'tourism',
        'active_submenu': 'events_add',
        'form': form,
        'title': 'Tambah Event Wisata',
    }
...
def admin_panel_event_update(request, event_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Terjadi kesalahan saat memperbarui event. Silakan periksa form.')
    else:
        form = TourismEventForm(instance=event)
...
@user_passes_test(is_staff)
def admin_panel_review_detail(request, review_id):
    """Detail review"""
    review = get_object_or_404(
        TourismReview.objects.select_related('tourism_location', 'user'), 
        id=review_id
    )
    ...
    context = {
        'review': review,
        'active_menu': 'tourism',
        'active_submenu': 'tourism_reviews',
    }
    return render(request, 'admin_panel/tourism/review_detail.html', context)

@user_passes_test(is_staff)
def admin_panel_review_edit(request, review_id):
    """Edit review"""
    review = get_object_or_404(TourismReview, id=review_id)
    
    if request.method == 'POST':
        form = TourismReviewForm(request.POST, instance=review)
        if form.is_valid():
            form.save()
            messages.success(request, 'Review berhasil diupdate!')
            return redirect('admin_panel:tourism_review_detail', review_id=review.id)
        else:
            messages.error(request, 'Terjadi kesalahan saat mengupdate review.')
    else:
        form = TourismReviewForm(instance=review)
    ...
    return render(request, 'admin_panel/tourism/review_form.html', context)
...
def admin_panel_gallery_list(request):
    ...
    # Base queryset - hanya gallery yang memiliki gambar
    galleries = TourismGallery.objects.select_related('tourism_location').exclude(image='')
    ...
    # Apply sorting
    galleries = galleries.order_by(sort_by)
    
    # Calculate statistics efficiently
    total_images = TourismGallery.objects.count()
    active_images = TourismGallery.objects.filter(is_active=True).count()
    featured_images = TourismGallery.objects.filter(is_featured=True).count()
    
    # Calculate total size efficiently using database aggregation
    from django.db.models import Sum, Case, When, IntegerField
    total_size = TourismGallery.objects.aggregate(
        total_size=Sum(
            Case(
                When(image__isnull=False, then='image__size'),
                default=0,
    ...
    context = {
        ...
        'images': page_obj,  # Template expects 'images'
        'page_obj': page_obj,
        'locations': locations,
        'location_filter': location_filter,
        'search_query': search_query,
...
@user_passes_test(is_staff)
def admin_panel_gallery_create(request):
    """Create gallery baru"""
    from .forms import TourismGalleryForm
    
    if request.method == 'POST':
        form = TourismGalleryForm(request.POST, request.FILES)
        if form.is_valid():
            gallery = form.save()
            messages.success(request, '✅ Gallery berhasil ditambahkan!')
            return redirect('admin_panel:tourism_gallery_list')
        # Error akan ditampilkan di form, tidak perlu messages.error
    else:
        form = TourismGalleryForm()
    
    context = {
        'form': form,
        'active_menu': 'tourism',
        'active_submenu': 'tourism_gallery',
    }
    return render(request, 'admin_panel/tourism/gallery_form.html', context)

@user_passes_test(is_staff)
def admin_panel_gallery_edit(request, gallery_id):
    """Edit gallery"""
    from .forms import TourismGalleryForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, '✅ Gallery berhasil diupdate!')
            return redirect('admin_panel:tourism_gallery_list')
        # Error akan ditampilkan di form, tidak perlu messages.error
    else:
        form = TourismGalleryForm(instance=gallery)
...
@user_passes_test(is_staff)
def admin_panel_faq_create(request):
    """Create FAQ baru"""
    from .forms import TourismFAQForm
    
    if request.method == 'POST':
        form = TourismFAQForm(request.POST)
        if form.is_valid():
            faq = form.save()
            messages.success(request, '✅ FAQ berhasil ditambahkan!')
            return redirect('admin_panel:tourism_faq_detail', faq_id=faq.id)
    else:
        form = TourismFAQForm()
    ...
    context = {
        'form': form,
        'active_menu': 'tourism',
        'active_submenu': 'tourism_faq',
    }
    return render(request, 'admin_panel/tourism/faq_form.html', context)

@user_passes_test(is_staff)
def admin_panel_faq_detail(request, faq_id):
    """Detail FAQ"""
    faq = get_object_or_404(TourismFAQ, id=faq_id)
    ...
    return render(request, 'admin_panel/tourism/faq_detail.html', context)

@user_passes_test(is_staff)
def admin_panel_faq_edit(request, faq_id):
    """Edit FAQ"""
    from .forms import TourismFAQForm
    ...
    if request.method == 'POST':
        form = TourismFAQForm(request.POST, instance=faq)
        if form.is_valid():
            form.save()
            messages.success(request, '✅ FAQ berhasil diupdate!')
            return redirect('admin_panel:tourism_faq_detail', faq_id=faq.id)
    else:
        form = TourismFAQForm(instance=faq)
...
@user_passes_test(is_staff)
@require_POST
def admin_panel_faq_delete(request, faq_id):
    """Delete FAQ"""
    faq = get_object_or_404(TourismFAQ, id=faq_id)
    faq.delete()
...
def admin_panel_reports(request):
    ...
    # Category distribution untuk chart
    category_stats = TourismLocation.objects.values('category__name').annotate(
        count=Count('id')
    ).order_by('-count')
    
    category_labels = json.dumps([stat['category__name'] or 'Tanpa Kategori' for stat in category_stats])
    category_data = json.dumps([stat['count'] for stat in category_stats])
    
    # Monthly review statistics untuk chart
    monthly_stats = []
    ...
    monthly_labels = json.dumps([stat['month'] for stat in monthly_stats])
    monthly_data = json.dumps([stat['count'] for stat in monthly_stats])
    
    # Top locations
    from django.db.models.functions import Coalesce
    from django.db.models import Value
    
    top_locations = TourismLocation.objects.annotate(
        review_count=Count('reviews'),
        avg_rating=Coalesce(Avg('ratings__rating'), Value(0.0))
    ).order_by('-review_count')[:10]
    
    # Recent reviews
    recent_reviews = TourismReview.objects.select_related('user', 'tourism_location').order_by('-created_at')[:5]
...
def admin_panel_settings(request):
    ...
    if request.method == 'POST':
        # Get or create settings in cache
        settings_key = 'tourism_settings'
        settings = cache.get(settings_key) or {}
        
        # Update all settings from POST
        settings.update({
            'site_name': request.POST.get('site_name', 'Desa Pulosari'),
            ...
            'enable_reviews': request.POST.get('enable_reviews') == 'on',
            'enable_ratings': request.POST.get('enable_ratings') == 'on',
            'default_latitude': request.POST.get('default_latitude', '-7.250445'),
            'default_longitude': request.POST.get('default_longitude', '110.214845'),
            'map_zoom': int(request.POST.get('map_zoom', 13)),
            'google_maps_api_key': request.POST.get('google_maps_api_key', ''),
            'notify_new_review': request.POST.get('notify_new_review') == 'on',
            'notify_new_rating': request.POST.get('notify_new_rating') == 'on',
            'notification_email': request.POST.get('notification_email', ''),
            'auto_approve_reviews': request.POST.get('auto_approve_reviews') == 'true',
        ...
        # Save to cache
        cache.set(settings_key, settings, None)  # Never expire
        
        # Update dashboard configuration
        if dashboard_config:
            dashboard_config.title = request.POST.get('site_name', dashboard_config.title)
            dashboard_config.description = request.POST.get('site_description', dashboard_config.description)
    ...
    settings = cache.get('tourism_settings') or {
        'site_name': 'Desa Pulosari',
        'site_description': 'Portal wisata desa Pulosari yang menampilkan berbagai destinasi wisata menarik.',
        'items_per_page': 20,
        'enable_reviews': True,
        'enable_ratings': True,
        'default_latitude': '-7.250445',
        'default_longitude': '110.214845',
        'map_zoom': 13,
