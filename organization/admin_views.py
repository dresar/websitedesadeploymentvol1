...
from .forms import LembagaAdatForm, PenggerakPKKForm, KepemudaanForm, KarangTarunaForm
...
def admin_organization_list(request):
    """Admin Panel - Dashboard utama organisasi"""
    ...
    return render(request, 'admin_panel/organization/dashboard.html', context)
...
def admin_pkk_list(request):
    ...
    pkk = PenggerakPKK.objects.select_related('penduduk').filter(
        status=status_filter
    )
...
def perangkat_desa_create(request):
    """Create new Perangkat Desa - Redirect to village_profile"""
    # Perangkat Desa sekarang dikelola di village_profile app
    return redirect('village_profile:admin_official_add')


@login_required
def perangkat_desa_edit(request, pk):
    """Edit Perangkat Desa - Redirect to village_profile"""
    # Perangkat Desa sekarang dikelola di village_profile app
    return redirect('village_profile:admin_official_edit', official_id=pk)
...
def lembaga_adat_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                lembaga = form.save()
                messages.success(request, 'Lembaga Adat berhasil diperbarui')
                return redirect('admin_panel:organization:admin_lembaga_adat_detail', pk=lembaga.id)
            except Exception as e:
                messages.error(request, f'Terjadi kesalahan: {str(e)}')
        else:
    ...
    context = {
        'page_title': f'Edit {lembaga.nama_lembaga}',
        'meta_description': f'Edit data {lembaga.nama_lembaga}',
        'form': form,
        'lembaga': lembaga,
        'active_menu': 'organization',
        'active_submenu': 'lembaga_adat'
...
@login_required
@require_http_methods(["GET"])
def api_admin_pkk_list(request):
    """API endpoint for admin PKK list with pagination"""
    try:
        search_query = request.GET.get('search', '')
        jabatan_filter = request.GET.get('jabatan', '')
        status_filter = request.GET.get('status', 'aktif')
        ...
        pkk_page = pkk[start:end]
        
        # Build response data
        data = []
        for item in pkk_page:
            data.append({
                'id': item.id,
                'nama': item.penduduk.name,  # references.Penduduk uses 'name' not 'nama'
                'nik': item.penduduk.nik,
                'jabatan': item.jabatan,
                'jabatan_display': item.get_jabatan_display(),
                'foto_profil': item.foto_profil.url if item.foto_profil else None,
                'tanggal_bergabung': item.tanggal_bergabung.strftime('%Y-%m-%d') if item.tanggal_bergabung else None,
                'tanggal_mulai_tugas': item.tanggal_mulai_tugas.strftime('%Y-%m-%d') if item.tanggal_mulai_tugas else None,
                'status': item.status,
                'keahlian': item.keahlian,
                'kontak_whatsapp': item.kontak_whatsapp,
                'email': item.email,
            })
...
@login_required
@require_http_methods(["GET"])
def api_admin_karang_taruna_list(request):
    """API endpoint for admin Karang Taruna list with pagination"""
    try:
        search_query = request.GET.get('search', '')
        jabatan_filter = request.GET.get('jabatan', '')
        status_filter = request.GET.get('status', 'aktif')
        pengurus_inti_filter = request.GET.get('pengurus_inti', '')
        page = int(request.GET.get('page', 1))
        per_page = int(request.GET.get('per_page', 10))
        
        # Base queryset
        kt = KarangTaruna.objects.select_related('penduduk').filter(
            status=status_filter
        )
        ...
        # Apply jabatan filter
        if jabatan_filter:
            kt = kt.filter(jabatan=jabatan_filter)
        
        # Apply pengurus inti filter
        if pengurus_inti_filter == 'true':
            kt = kt.filter(is_pengurus_inti=True)
        elif pengurus_inti_filter == 'false':
            kt = kt.filter(is_pengurus_inti=False)
        ...
        for item in kt_page:
            data.append({
                ...
                'is_pengurus_inti': item.is_pengurus_inti,
            })
