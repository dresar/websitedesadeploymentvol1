...
def import_export_dashboard(request):
    ...
    if os.path.exists(templates_dir):
        ...
        for model_name, template_file in template_files.items():
    ...
    context = {
        ...
        'active_submenu': 'import_export_dashboard',
    }
...
def download_template(request, model_name, format_type='excel'):
    """Download template untuk import data dari folder templates_import/excel_templates/"""
    try:
        import os
        from django.conf import settings
        ...
        if model_name not in template_files:
            messages.error(request, f'Template untuk {model_name} tidak tersedia')
            return redirect('admin_panel:import_export_dashboard')
        ...
        if not os.path.exists(template_path):
            messages.error(request, f'File template {template_file} tidak ditemukan')
            return redirect('admin_panel:import_export_dashboard')
...
@login_required
def global_import(request):
    """Global import untuk semua jenis data dalam satu file"""
    if request.method == 'GET':
        # Tampilkan form global import
        context = {
            'title': 'Global Import Data',
            'formats': ['excel', 'csv', 'json'],
            'supported_models': [
                {'name': 'penduduk', 'verbose_name': 'Data Penduduk'},
                {'name': 'dusun', 'verbose_name': 'Data Dusun'},
                {'name': 'lorong', 'verbose_name': 'Data Lorong'},
                {'name': 'rw', 'verbose_name': 'Data RW'},
                {'name': 'rt', 'verbose_name': 'Data RT'},
                {'name': 'keluarga', 'verbose_name': 'Data Keluarga'},
                {'name': 'family', 'verbose_name': 'Data Family'},
                {'name': 'pelajar', 'verbose_name': 'Data Pelajar'},
                {'name': 'disabilitas', 'verbose_name': 'Data Disabilitas'},
                {'name': 'disabilitas_type', 'verbose_name': 'Jenis Disabilitas'},
                {'name': 'religion', 'verbose_name': 'Data Agama'},
            ],
            # Menu context untuk sidebar
            'active_menu': 'references',
            'active_submenu': 'global_import',
        }
        return render(request, 'admin_panel/references/global_import_form.html', context)
    
    else:
        # Handle global import request
        format_type = request.POST.get('format', 'excel')
        validate_only = request.POST.get('validate_only', 'off') == 'on'
        
        if 'file' not in request.FILES:
            messages.error(request, 'Tidak ada file yang dipilih')
            return redirect('references:global_import')
        
        file = request.FILES['file']
        print(f"File received: {file.name}, size: {file.size}")
        
        # Simpan file yang diupload
        file_path = os.path.join(settings.MEDIA_ROOT, 'temp', f'global_import_{datetime.now().strftime("%Y%m%d_%H%M%S")}.{format_type}')
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        ...
        try:
            manager = ImportExportManager()
            result = manager.global_import_data(file_path, format_type, validate_only)
            
            if result['success']:
                if validate_only:
                    messages.success(request, f'Validasi berhasil! {result["total_rows"]} baris data valid.')
                else:
                    messages.success(request, f'Global import berhasil! {result["imported"]} data berhasil diimpor dari {result["total_rows"]} baris.')
                    
                    # Tampilkan detail hasil per model
                    if 'results' in result:
                        for model_name, model_result in result['results'].items():
                            if model_result['imported'] > 0:
                                messages.info(request, f'{model_name.title()}: {model_result["imported"]} data diimpor')
            else:
                messages.warning(request, f'Global import selesai dengan {len(result["errors"])} error.')
                for error in result['errors'][:5]:  # Tampilkan 5 error pertama
                    messages.error(request, f'Baris {error["row"]}: {error["error"]}')
            ...
            return redirect('references:import_export_dashboard')
...
def list_available_templates(request):
    """Get list of available templates dari folder templates_import/excel_templates/"""
    try:
        import os
        from django.conf import settings
        ...
        template_files = {
            'penduduk': {
                ...
                'name': 'Template Import Penduduk',
                'description': 'Template untuk import data penduduk desa'
            },
            'dusun': {
                'file': 'template_import_dusun.xlsx',
                'name': 'Template Import Dusun',
                'description': 'Template untuk import data dusun/hamlet'
            },
            'lorong': {
                'file': 'template_import_lorong.xlsx',
                'name': 'Template Import Lorong',
                'description': 'Template untuk import data lorong/gang'
            },
            'rw': {
                'file': 'template_import_rw.xlsx',
                'name': 'Template Import RW',
                'description': 'Template untuk import data Rukun Warga'
            },
            'rt': {
                'file': 'template_import_rt.xlsx',
                'name': 'Template Import RT',
                'description': 'Template untuk import data Rukun Tetangga'
            },
            'keluarga': {
                'file': 'template_import_keluarga.xlsx',
                'name': 'Template Import Keluarga',
                'description': 'Template untuk import data keluarga'
            },
            'pelajar': {
                'file': 'template_import_pelajar.xlsx',
                'name': 'Template Import Pelajar',
                'description': 'Template untuk import data pelajar/mahasiswa'
            },
            'disabilitas': {
                'file': 'template_import_disabilitas.xlsx',
                'name': 'Template Import Disabilitas',
                'description': 'Template untuk import data penyandang disabilitas'
            },
        }
        ...
        if os.path.exists(templates_dir):
            for model_name, template_info in template_files.items():
                ...
                if os.path.exists(template_path):
                    ...
                    available_templates.append({
                        ...
                        'file_size': file_size,
                        'download_url': f'/admin-panel/references/download-template/{model_name}/'
                    })
...
def get_import_template(request, model_name):
    """Get import template untuk model tertentu dari folder templates_import/excel_templates/"""
    try:
        import os
        from django.conf import settings
        ...
        if model_name not in template_files:
            return JsonResponse({'error': f'Template untuk {model_name} tidak tersedia'}, status=404)
        
        template_file = template_files[model_name]
        template_path = os.path.join(templates_dir, template_file)
        
        if not os.path.exists(template_path):
            return JsonResponse({'error': f'File template {template_file} tidak ditemukan'}, status=404)
...
def preview_excel(request):
    try:
        if 'file' not in request.FILES:
            return JsonResponse({'success': False, 'error': 'Tidak ada file yang dipilih'})
        
        file = request.FILES['file']
        
        # Validasi file type
        if not file.name.endswith(('.xlsx', '.xls')):
            return JsonResponse({'success': False, 'error': 'File harus berupa Excel (.xlsx atau .xls)'})
        
        # Simpan file sementara
        temp_path = os.path.join(settings.MEDIA_ROOT, 'temp', f'preview_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx')
        ...
        try:
            # Baca Excel file
            df = pd.read_excel(temp_path, sheet_name=0)  # Ambil sheet pertama
            
            # Konversi ke format yang bisa ditampilkan
            headers = df.columns.tolist()
            rows = []
            
            # Ambil maksimal 50 baris untuk preview
            max_rows = min(len(df), 50)
            for i in range(max_rows):
                row_data = []
                for col in headers:
                    value = df.iloc[i][col]
                    # Konversi nilai NaN ke string kosong
                    if pd.isna(value):
                        value = ''
                    else:
                        value = str(value)
            ...
            return JsonResponse({'success': False, 'error': f'Error membaca file Excel: {str(e)}'})
        ...
        logger.error(f"Error in preview_excel: {str(e)}")
