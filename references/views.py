...
from .forms import PendudukForm, DusunForm, LorongForm, RWForm, RTForm, KeluargaForm
...
def population_page(request):
    ...
    context = {
        ...
        'page_subtitle': 'Statistik dan demografi penduduk Desa Pulosarok',
...
def dusun_add(request):
    ...
    if request.method == 'POST':
        try:
            ...
            area_size = data.get('area_size', '').strip()
            if area_size:
                try:
...
def dusun_edit(request, dusun_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            dusun.code = code if code else None
            
            dusun.description = request.POST.get('description', dusun.description)
            
            # Handle area_size - optional field
            area_size = request.POST.get('area_size', '').strip()
            if area_size:
                try:
                    dusun.area_size = float(area_size.replace(',', '.'))
                except (ValueError, TypeError):
                    dusun.area_size = None
            else:
                dusun.area_size = None
            
            # Handle kepala_dusun
            kepala_dusun_id = request.POST.get('kepala_dusun', '').strip()
            if kepala_dusun_id:
                try:
...
def api_dusun_list(request):
    ...
    try:
        ...
        for dusun in dusun_list:
            ...
            nama = str(dusun.name) if dusun.name else '-'
            kode = str(dusun.code) if dusun.code else '-'
            kepala_dusun_name = str(dusun.kepala_dusun.name) if dusun.kepala_dusun else '-'
            description = str(dusun.description) if dusun.description else ''
            alamat_kantor = str(getattr(dusun, 'alamat_kantor', '')) if getattr(dusun, 'alamat_kantor', '') else ''
            ...
            results.append({
                ...
                'kepala_dusun': kepala_dusun_name,
                'jumlah_rw': int(rw_count),
                'population_count': int(population_count),
                'is_active': bool(dusun.is_active),
        ...
        import traceback
        print(f"Error in api_dusun_list: {str(e)}")
        print(traceback.format_exc())
        return JsonResponse({
            'success': False,
            'error': str(e),
            'message': 'Terjadi kesalahan saat memuat data dusun'
        }, status=500)
...
def lorong_add(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if request.content_type == 'application/json':
                data = json.loads(request.body)
                dusun_id = data.get('dusun_id')
            else:
                data = request.POST
                dusun_id = data.get('dusun')
            
            # Required fields validation
            if not data.get('nama_lorong') or not dusun_id:
                if request.content_type == 'application/json':
                    return JsonResponse({
                        'success': False,
                        'message': 'Nama lorong dan dusun harus diisi'
                    }, status=400)
                else:
                    messages.error(request, 'Nama lorong dan dusun harus diisi')
                    return redirect('admin_panel:references_lorong_add')
            
            dusun = get_object_or_404(Dusun, id=dusun_id)
            
            # Handle optional fields
            kode = data.get('kode', '').strip()
            ketua_lorong = data.get('ketua_lorong', '').strip()
            rt_number = data.get('rt_number', '').strip()
            length = data.get('length', '').strip()
            
            lorong = Lorong.objects.create(
                dusun=dusun,
                nama_lorong=data['nama_lorong'],
                kode=kode if kode else None,
                ketua_lorong=ketua_lorong if ketua_lorong else None,
                rt_number=rt_number if rt_number else None,
                description=data.get('description', ''),
            ...
            if request.content_type == 'application/json':
                ...
                messages.success(request, 'Data lorong berhasil ditambahkan.')
...
def lorong_edit(request, lorong_id):
    ...
    try:
        ...
        if request.method == 'POST':
            # Redirect to lorong_update for POST handling
            return lorong_update(request, lorong_id)
...
def lorong_update(request, lorong_id):
    ...
    if request.method == 'POST':
        try:
            ...
            lorong.kode = kode if kode else None
            lorong.ketua_lorong = ketua_lorong if ketua_lorong else None
            lorong.rt_number = rt_number if rt_number else None
            lorong.description = data.get('description', '')
            lorong.length = float(length) if length else None
            ...
            if request.content_type == 'application/json':
                ...
                messages.success(request, 'Data lorong berhasil diperbarui.')
                return redirect('admin_panel:references_lorong_list')
            ...
            if request.content_type == 'application/json':
                ...
                messages.error(request, f'Error: {str(e)}')
                return redirect('admin_panel:references_lorong_edit', lorong_id=lorong_id)
...
def keluarga_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Data tidak valid. Periksa kembali form.')
    else:
        form = KeluargaForm()
...
def disabilitas_list(request):
    ...
    if severity_filter:
        disabilitas_list = disabilitas_list.filter(severity=severity_filter)
    
    # Apply status filter
    if status_filter:
        if status_filter == 'aktif':
            disabilitas_list = disabilitas_list.filter(is_active=True)
        elif status_filter == 'tidak_aktif':
            disabilitas_list = disabilitas_list.filter(is_active=False)
...
def api_search_residents(request):
    """API for searching residents with optional age filter - accessible by other apps"""
    try:
        query = request.GET.get('q', '').strip()
        dusun_id = request.GET.get('dusun_id')
        page = int(request.GET.get('page', 1))
        per_page = int(request.GET.get('per_page', 20))
        age_filter = request.GET.get('age_filter', 'true').lower() == 'true'  # Default to true for backward compatibility
        no_pagination = request.GET.get('no_pagination', 'false').lower() == 'true'  # New parameter to disable pagination
        ...
        # Apply age filter only if requested (5-35 years)
        if age_filter:
        ...
        # Handle pagination based on no_pagination parameter
        if no_pagination:
            ...
            return JsonResponse({
                'success': True,
                'data': residents_data,
                'total_count': len(residents_data),
                'pagination': {
                    'current_page': 1,
                    'total_pages': 1,
                    'total_items': len(residents_data),
                    'has_next': False,
                    'has_previous': False
                }
            })
...
def api_public_population_demographics(request):
    ...
    try:
        ...
        for stat in education_stats:
            edu = stat['education']
            count = stat['count']
            percentage = (count / total_with_education * 100) if total_with_education > 0 else 0
            education[edu] = {
                'count': count,
                'percentage': round(percentage, 1)
            }
...
def penduduk_save(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Handle photo upload - SIMPLE VERSION
...
def rw_create(request):
    ...
    context = {
        'form_title': 'Tambah RW',
        'form_subtitle': 'Tambahkan data Rukun Warga (RW) baru',
        'title': 'Tambah RW',
        'form': form,
    }
...
def rw_edit(request, rw_id):
    """Edit RW"""
    try:
        rw = RW.objects.select_related('dusun', 'ketua_rw').get(id=rw_id)
    ...
    context = {
        'form_title': 'Edit RW',
        'form_subtitle': 'Perbarui data Rukun Warga (RW) yang sudah ada',
        'title': 'Edit RW',
        'form': form,
        'rw': rw,
    }
...
def rw_delete(request, rw_id):
    ...
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({
                'success': True,
                'message': f'RW {rw_number} berhasil dihapus'
            })
        except RW.DoesNotExist:
            return JsonResponse({
                'success': False,
                'error': 'RW tidak ditemukan'
            }, status=404)
        except Exception as e:
            return JsonResponse({
                'success': False,
...
def rt_list(request):
    ...
    rw_options = RW.objects.select_related('dusun').filter(is_active=True).order_by('dusun__name', 'rw_number')
    dusun_options = Dusun.objects.filter(is_active=True).order_by('name')
...
def rt_create(request):
    ...
    context = {
        'form_title': 'Tambah RT',
        'form_subtitle': 'Tambahkan data Rukun Tetangga (RT) baru',
        'title': 'Tambah RT',
        'form': form,
    }
...
def rt_edit(request, rt_id):
    ...
    context = {
        'form_title': 'Edit RT',
        'form_subtitle': 'Perbarui data Rukun Tetangga (RT) yang sudah ada',
        'title': 'Edit RT',
        'form': form,
        'rt': rt,
    }
...
def rt_delete(request, rt_id):
    ...
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({
                'success': True,
                'message': f'RT {rt_number} berhasil dihapus'
            })
        except RT.DoesNotExist:
            return JsonResponse({
                'success': False,
...
@login_required
def rt_detail(request, rt_id):
    """View detail RT"""
    try:
        rt = RT.objects.select_related('rw__dusun', 'ketua_rt').get(id=rt_id)
        context = {
            'rt': rt,
            'title': f'Detail RT {rt.rt_number}',
        }
        return render(request, 'admin_panel/references/rt_detail.html', context)
    except RT.DoesNotExist:
        messages.error(request, 'RT tidak ditemukan')
        return redirect('admin_panel:references_rt_list')

@login_required
def rw_detail(request, rw_id):
    """View detail RW"""
    try:
        rw = RW.objects.select_related('dusun', 'ketua_rw').get(id=rw_id)
        context = {
            'rw': rw,
            'title': f'Detail RW {rw.rw_number}',
        }
        return render(request, 'admin_panel/references/rw_detail.html', context)
    except RW.DoesNotExist:
        messages.error(request, 'RW tidak ditemukan')
        return redirect('admin_panel:references_rw_list')
...
def api_penduduk_list(request):
    ...
    try:
        # Check if kk_number parameter is provided
        kk_number = request.GET.get('kk_number')
        
        if kk_number:
            # Filter penduduk by kk_number
            penduduks = Penduduk.objects.filter(kk_number=kk_number, is_active=True).select_related('dusun', 'lorong')
        else:
...
def keluarga_bulk_delete(request):
    ...
    if request.method == 'POST':
        try:
            # Try both formats: keluarga_ids[] and keluarga_ids
            keluarga_ids = request.POST.getlist('keluarga_ids[]') or request.POST.getlist('keluarga_ids')
            ...
            if deleted_count == 0:
                return JsonResponse({'success': False, 'message': 'Tidak ada data yang ditemukan'})
...
def api_keluarga_list(request):
    ...
    try:
        ...
        results = [{
            ...
            'rw': keluarga.rw or '',
            'is_active': keluarga.is_active,
            'jumlah_anggota': Penduduk.objects.filter(kk_number=keluarga.nomor_kk, is_active=True).count(),
            'created_at': keluarga.created_at.isoformat() if keluarga.created_at else None,
            'updated_at': keluarga.updated_at.isoformat() if keluarga.updated_at else None
...
def api_penduduk_search(request):
    """API untuk mencari penduduk berdasarkan nama atau NIK"""
    try:
        query = request.GET.get('q', '').strip()
        ...
        # Search penduduk by name or NIK - simplified for debugging
        penduduks = Penduduk.objects.filter(
        ...
        for penduduk in penduduks:
            try:
                ...
                address_parts = []
                if hasattr(penduduk, 'house_number') and penduduk.house_number:
                    address_parts.append(f"No. {penduduk.house_number}")
                if hasattr(penduduk, 'rt_number') and penduduk.rt_number:
                    address_parts.append(f"RT {penduduk.rt_number}")
                if hasattr(penduduk, 'rw_number') and penduduk.rw_number:
                    address_parts.append(f"RW {penduduk.rw_number}")
                if hasattr(penduduk, 'lorong') and penduduk.lorong and hasattr(penduduk.lorong, 'name'):
                    address_parts.append(f"Lorong {penduduk.lorong.name}")
                if hasattr(penduduk, 'dusun') and penduduk.dusun and hasattr(penduduk.dusun, 'name'):
                    address_parts.append(f"Dusun {penduduk.dusun.name}")
                
                address = ", ".join(address_parts) if address_parts else "Alamat tidak tersedia"
                
                # Get contact information safely
                phone = ''
                if hasattr(penduduk, 'phone_number') and penduduk.phone_number:
                    phone = penduduk.phone_number
                elif hasattr(penduduk, 'mobile_number') and penduduk.mobile_number:
                    phone = penduduk.mobile_number
                
                
                results.append({
                    'id': penduduk.id,
                    'name': penduduk.name,  # Changed from 'nama' to 'name' to match JS expectations
                    'nik': penduduk.nik,
                    'address': address,  # Changed from 'alamat' to 'address' to match JS expectations
                    'phone': phone,  # Changed from 'telepon' to 'phone' to match JS expectations
                })
            except Exception as e:
                # Skip problematic records but log the error
                print(f"Error processing penduduk {penduduk.id}: {str(e)}")
                continue
