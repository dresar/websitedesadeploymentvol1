...
from .models import Dusun, Lorong, Penduduk, DisabilitasType, DisabilitasData, ReligionReference, Family, RW, RT, Keluarga
...
class RWForm(forms.ModelForm):
    class Meta:
        model = RW
        fields = ['dusun', 'rw_number', 'ketua_rw', 'description', 'is_active']
        widgets = {
            'description': forms.Textarea(attrs={'rows': 3}),
        }
        labels = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'dusun' in self.fields:
            ...
            if self.instance and self.instance.pk and self.instance.dusun:
                self.fields['dusun'].initial = self.instance.dusun.id
...
class RTForm(forms.ModelForm):
    class Meta:
        model = RT
        fields = ['rw', 'rt_number', 'ketua_rt', 'description', 'is_active']
        widgets = {
            'description': forms.Textarea(attrs={'rows': 3}),
        }
        labels = {
...
class KeluargaForm(forms.ModelForm):
    class Meta:
        model = Keluarga
        fields = ['dusun', 'nama_kepala_keluarga', 'nomor_kk', 'rt', 'rw', 'alamat', 'is_active']
        widgets = {
            'alamat': forms.Textarea(attrs={'rows': 3}),
            'nomor_kk': forms.TextInput(attrs={'maxlength': '16', 'pattern': '[0-9]{16}'}),
            'rt': forms.TextInput(attrs={'maxlength': '3'}),
            'rw': forms.TextInput(attrs={'maxlength': '3'}),
        }
        labels = {
            'dusun': 'Dusun',
    ...
    def clean_nomor_kk(self):
        nomor_kk = self.cleaned_data.get('nomor_kk')
        if nomor_kk:
            # Check KK number format (16 digits)
            if not nomor_kk.isdigit() or len(nomor_kk) != 16:
                raise ValidationError('Nomor KK harus berupa 16 digit angka.')
            
            # Check for duplicate KK
            queryset = Keluarga.objects.filter(nomor_kk=nomor_kk)
            if self.instance.pk:
                queryset = queryset.exclude(pk=self.instance.pk)
            if queryset.exists():
                raise ValidationError('Nomor KK sudah terdaftar dalam sistem.')
    ...
    def clean_rt(self):
        rt = self.cleaned_data.get('rt')
        if rt and not rt.isdigit():
            raise ValidationError('Nomor RT harus berupa angka.')
        return rt
    
    def clean_rw(self):
        rw = self.cleaned_data.get('rw')
        if rw and not rw.isdigit():
            raise ValidationError('Nomor RW harus berupa angka.')
        return rw