...
# from tinymce import urls as tinymce_urls  # DISABLED - tidak terinstall

# View untuk redirect admin login ke admin panel
def redirect_admin_login(request):
    """Redirect admin login ke admin panel"""
...
urlpatterns = [
    # Admin URLs - Redirect admin login ke admin panel
    path('admin/login/', redirect_admin_login, name='admin_login_redirect'),
    path('admin/', block_admin, name='block_admin'),  # Blokir akses admin bawaan lainnya
...
# Always serve static files when using runserver (even if DEBUG=False)
import sys
if 'runserver' in sys.argv or settings.DEBUG:
    urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)
    urlpatterns += static(settings.STATIC_URL, document_root=settings.STATIC_ROOT)
    
# Debug toolbar (only if DEBUG=True)
